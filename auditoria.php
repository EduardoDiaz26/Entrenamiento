...
<?php 
$auditoria = Database::Connect()->query("select a.*, u.usuario from auditoria a INNER JOIN usuarios u on a.usuario_id = u.id");
 ?>
 <div class="container">
<ul class="list-group"> 
...
<?php while ($actividades = $auditoria->fetch_object()):?>
  <li class="list-group-item">El usuario <?=$actividades->usuario.' '.$actividades->accion.' ('.$actividades->usuario_id.
                                        ...
                                        .', '.$actividades->cargo.', '.$actividades->creado.', '.$actividades->modificado.')'
                                        ?>  </li>
<?php endwhile; ?>
</ul>
</div>
